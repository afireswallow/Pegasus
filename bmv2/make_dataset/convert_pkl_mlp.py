...
parser = argparse.ArgumentParser(description='Convert PyTorch model to P4 tables and test')
parser.add_argument('--model_path', type=str, required=True, 
                   help='Path to the PyTorch model file (.pt)')
parser.add_argument('--test_data_path', type=str, required=True,
                   help='Path to the test data CSV file')
parser.add_argument('--output_pkl_path', type=str, 
                   help='Path to save the output pickle file (required unless --test_only)')
parser.add_argument('--dataset_pkl_path', type=str,
                   help='Path to save the dataset pickle file (required unless --test_only)')
parser.add_argument('--dataset_type', type=str, choices=['CICIOT2022', 'PeerRush', 'ISCXVPN'], 
                   required=True, default='CICIOT2022' ,help='Type of dataset for different label processing')
parser.add_argument('--device', type=str, choices=['auto', 'cpu', 'cuda'], default='auto',
                   help='Device to use for model loading (auto: detect automatically, cpu: force CPU, cuda: force GPU)')
parser.add_argument('--test_only', action='store_true',
                   help='Only run testing without saving model tables and dataset files')

args = parser.parse_args()


if not args.test_only:
    if not args.output_pkl_path:
        parser.error("--output_pkl_path is required when not using --test_only")
    if not args.dataset_pkl_path:
        parser.error("--dataset_pkl_path is required when not using --test_only")
...
def from_S_T_H_to_range2(S, T, H):
    cc1 = [] ; cc2 = []
    for i in range(T.shape[0]):
        t1 = [] ; t2 = [] ; u1 = [] ; u2 = [] ; c1 = [] ; c2 = []
        for j in range(16):
...
def from_range_to_ternary(tt, bitLengthList, LUT, signed = 1, returnCategory = False, reservedOperation = 1): 
...
def from_tree_node_to_ternary(m, n, bitLength, category, key, LUT, returnCategory = False): #
    mask = "0"
...
def from_range2_to_ternary2(tt1, tt2, bitLengthList1, bitLengthList2, LUT, signed = 1, returnCategory = True, reservedOperation = 1): 
    bitLength1 = bitLengthList1
    bitLength2 = bitLengthList2
    if signed == 1: # signed
        min1 = -(1 << (bitLength1 - 1)) - 1
        max1 = (1 << (bitLength1 - 1)) - 1
    elif signed == 0: # unsigned
        min1 = -1
        max1 = (1 << bitLength1) - 1
    if signed == 1: 
    ...
    for i in range(len(ternary1)):
        ...
        ternary.append(s)
        usedCategoryList.append(usedCategory)
    #[Priority，match_item1，match_item2，mask1，mask2，category]
    return (ternary, usedCategoryList) if returnCategory else ternary
...
bitLengthOfFeature = [8,16,8,8,8,16,16,32,32,32]
T1 = model['MM1.T']
H = model['MM1.H']
LUT1 = model['MM1.LUT']
...
# The following code saves the model tables and dataset to specified paths.
if not args.test_only:
    with open(args.output_pkl_path, 'wb') as f:
        pickle.dump([table1, table2_0_1, table2_1_16, table2_3, table3], f, protocol=2)
    print(f"Model tables saved to: {args.output_pkl_path}")
else:
    print("Test-only mode: Model tables not saved")
...
with open(args.test_data_path, mode='r', encoding='utf-8') as file:
...
if not args.test_only:
    with open(args.dataset_pkl_path, 'wb') as f:
        pickle.dump([features, labels], f, protocol=2)
    print(f"Dataset saved to: {args.dataset_pkl_path}")
else:
    print("Test-only mode: Dataset not saved")
...
for i in range(len(features)):
    ...
    if args.dataset_type == 'CICIOT2022':
        label = 0
        max = ct[0]
        if ct[1] > max:
            label = 1
        ...
        if ct[2] > max:
            label = 2
    elif args.dataset_type == 'PeerRush':
        label = 0
        max = ct[0]
        if ct[1] > max:
            label = 1
        ...
        if ct[2] > max:
            label = 2
    elif args.dataset_type == 'ISCXVPN':
        label = 0
        max = ct[0]
        if ct[1] > max:
            label = 1
