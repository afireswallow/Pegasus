...
model.len_embedding.weight.requires_grad = False
model.ipd_embedding.weight.requires_grad = False
...
while True:
    ...
    epoch+=1
    status = early_stop(train_loss)
    modelname = "AUTOencoder"
    num = 1
    txt = 'modelnum: {}{}|lr:{}|epoch: {}|trainloss: {:.4f}'.format(
                modelname,num,originallr,epoch,train_loss
            )
    print(txt)
torch.save(model.state_dict(), f"{savepth}/autoencoder_mm_{epoch}e.pt")
